...
func initialize(view_data: Dictionary) -> void:
	if not BattleManager.battle_state_changed.is_connected(_on_battle_state_changed):
...
func _update_skill_buttons() -> void:
	...
	var skills: Array[Dictionary] = BattleManager.get_available_skills_for_ui(actor_id)
	if skills.is_empty():
		var label := Label.new()
		label.text = "没有可用的技能"
...
func _update_item_buttons() -> void:
	...
	var consumables: Array[Dictionary] = BattleManager.get_consumable_items_for_ui(
