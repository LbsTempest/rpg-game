...
var shop_inventories: Dictionary:
	get:
		return _shop_state().shop_inventories
	set(value):
		_shop_state().shop_inventories = value
...
func _initialize_shops() -> void:
	shop_definitions = ContentDB.get_all_shops()
...
func _on_item_selected(item_id: String, item_data: Dictionary) -> void:
	...
	if _ui_controller.is_buying():
		...
		_ui_controller.update_description(
			"%s\n出售价格: %d 金币/个\n拥有: %d"
			% [item_data.get("description", ""), price, item_data.get("quantity", 1)]
		)
...
func get_shop_inventory(shop_id: String) -> Dictionary:
	ensure_shop_inventory(shop_id)
	return shop_inventories.get(shop_id, {}).duplicate(true)

func _get_item_data(item_id: String) -> Dictionary:
	return ContentDB.get_item_definition(item_id)

func get_save_data() -> Dictionary:
	return _shop_state().to_save_data()

func load_save_data(data: Dictionary) -> void:
	_shop_state().load_save_data(data)
