...
func _initialize_skills_database() -> void:
	var skills_data = load("res://resources/data/skills_data.gd").new()
	available_skills = skills_data.skills_database.duplicate(true)
	skills_data.free()
...
func get_learned_skills() -> Array[Dictionary]:
	...
	for skill_id in learned_skills:
		if available_skills.has(skill_id):
			var skill_data = available_skills[skill_id].duplicate()
